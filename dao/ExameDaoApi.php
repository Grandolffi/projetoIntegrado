...
class ExameDao {
    public function inserir(ResultadoExames $exame) {
        ...
        $dados = [
            ...
            "data_hora_exame" => $exame->getDataHoraExame()
        ];
    ...
    public function editar(ResultadoExames $exame) {
        ...
        // Ajuste esta URL se seu endpoint for diferente (ex: http://localhost:3000/api/exames/)
        $url = "http://localhost:3000/editarexame/" . urlencode($exame->getId());
    ...
    public function listaExames($row){
        ...
        $exame->setId(htmlspecialchars($row['id'] ?? ''));
        $exame->setLaudoId(htmlspecialchars($row['laudo_id'] ?? ''));
        $exame->setNomeExame(htmlspecialchars($row['nome_exame'] ?? ''));
        $exame->setTipoExame(htmlspecialchars($row['tipo_exame'] ?? ''));
        $exame->setValorAbsoluto(htmlspecialchars($row['valor_absoluto'] ?? ''));
        $exame->setValorReferencia(htmlspecialchars($row['valor_referencia'] ?? ''));
        $exame->setPacienteRegistro(htmlspecialchars($row['paciente_registro'] ?? ''));
        $exame->setDataHoraExame(htmlspecialchars($row['data_hora_exame'] ?? ''));
        return $exame;
    } 
